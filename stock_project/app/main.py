...
def _calc_limit_up_price(prev_close):
    if pd.isna(prev_close):
        return None
    # Main-board and most A-shares 10%; STAR/ChiNext 20% cases are ignored in this simple version.
    return round(float(prev_close) * 1.10, 2)



def screen_limit_up_high_shrink_volume(
    trade_date: str,
    trading_days: int = 60,
    adj: str = "qfq",
    include_name: bool = True,
    ...
    if trading_days not in (60, 120, 250):
        raise HTTPException(status_code=400, detail="trading_days must be one of: 60, 120, 250")
    if adj not in ("qfq", "hfq", "none"):
        raise HTTPException(status_code=400, detail="adj must be one of: none, qfq, hfq")
    if limit_up_lookback_days < 1 or limit_up_lookback_days > 250:
...
@app.get("/screen/limit-up-high-shrink-volume")
def api_screen_limit_up_high_shrink_volume(
    trade_date: str,
    trading_days: int = Query(60, description="新高窗口: 60/120/250"),
    adj: str = Query("qfq", description="复权方式: qfq/hfq/none"),
    include_name: bool = Query(True, description="是否合并股票名称"),
    limit_up_lookback_days: int = Query(20, description="从交易日向前追溯天数"),
):
    result_df = screen_limit_up_high_shrink_volume(
        trade_date=trade_date,
        trading_days=trading_days,
        adj=adj,
        include_name=include_name,
    ...
    return {
        "message": "screen limit up high shrink volume ok",
        "trade_date": trade_date,
        "trading_days": trading_days,
        "adj": adj,
        "limit_up_lookback_days": limit_up_lookback_days,
